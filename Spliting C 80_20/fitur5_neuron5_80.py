...
train = pd.read_csv('Training_80_f5_keju.csv')
train.head(196)

test = pd.read_csv('Testing_20_f5_keju.csv')
test.head(49)

scaler = MinMaxScaler()
X_train = scaler.fit_transform(train.values[:,1:6])
y_train = scaler.fit_transform(train.values[:,6:])

X_test = scaler.fit_transform(test.values[:,1:6])
y_test = scaler.fit_transform(test.values[:,6:])
